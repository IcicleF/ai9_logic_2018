...
8. 重生阶段
  * **04.15更新：**死亡惩罚结束的玩家随机取代地图上的一个村民（该村民直接消失，并且玩家出生在这个村民原先所在的位置）；如果地图上不幸没有村民，那么就在随机位置出生；
  * 村民以 0.05 的概率在随机位置出生；如果成功出生一位村民，那么重复上述过程，否则停止重生村民。
  	* 0.05 概率的产生方式如下：生成一个服从 [-1, 19) 的均匀分布的随机实数，小于 0 则成功；
...
**04.15更新：**在游戏开始时，每名玩家都将获得 15 初始金钱。
...
* 使用守卫不受地形影响，守卫可以正常地放置在离玩家距离不超过 3 的任何一个位置，提供的视野范围不受地形阻拦；
