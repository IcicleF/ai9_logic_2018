...
在有至少一位玩家的积分达到 20 分或以上，或者回合数达到 1000 时，将会产生胜利者。
...
* **速度**：单位的速度向量；由于游戏机制的性质，该向量的模只能为 0 或 1.2。
...
另外，玩家每 5 回合还将获得 1 金钱。金钱的自然增长与玩家是否死亡无关。

## 移动

游戏中单位可以移动。移动在回合最后、单位重生之前结算。

所有的移动都是分段的匀速直线运动，移动速度只能是每回合 1.2 单位长度。虽然这么说，因为是回合制，所以实质上都是“瞬移”。
...
村民的移动规律如下：任意选取地图上一个合法位置，沿最短路径前往彼处，原地停止 15 ~ 30 回合，然后重复。不过，在村民重生后的第一次移动之前，有 1/2 的概率先原地静止不动 1 ~ 7 回合，再开始第一次移动。

## 昼夜和视野系统

如您所知，游戏中存在昼夜系统。良好利用昼夜的特性，在白天隐藏自己、收集信息，在黑夜亮出獠牙，是取胜的关键。

* 白昼长度为 80 回合，黑夜长度为 60 回合；
...
* 每 10 回合，受到 6 点伤害。**所谓“每 10 回合”，是指玩家在编号为 10 的倍数，同时又是白昼的回合受到伤害（首回合的编号为 1）。**

## 普通攻击

玩家可以对身边的单位进行普通攻击。普通攻击造成 50 点伤害，并且回复自身 35 点 HP。
...
* 30 回合内处于死亡状态，一切指令无效，并且丧失玩家自身的视野（守卫不受影响）；
* 所持金钱乘以 0.7 向下取整。
