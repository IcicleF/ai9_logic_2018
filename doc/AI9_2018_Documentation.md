...
如您仍有疑问没有得到解答，或者认为本文档中有明显的错误，抑或是认为逻辑的运作方式明显不合理，欢迎您在智能体大赛QQ群中提出，或者联系<a href="mailto:icicle.flw@gmail.com">icicle.flw@gmail.com</a>

### 04.15 更新日志

Bug修复：

* 修复了浮点误差导致 reachable 函数返回错误结果的问题
* 修复了移动时速度可能超过 2.0 的问题
* 修复了游戏机制上的漏洞，现在玩家重生时会“附身”一个村民

我们还重新评估了游戏中的各种数值，并作出了一些修改：

* **普通攻击的吸血**：10 → 12
* **普通攻击的 CD**：15 → 12
* **炸弹的价格**：20 → 25
* **炸弹的 CD**：40 → 60
* **放置守卫的范围（施法距离）**：3 → 4（与普通攻击相同）
* **玩家的初始金钱**：0 → 15

我们注意到，现在，随着游戏的进行，村民数量很可能逐渐减少以至于被杀光。针对这一现象作出的调整如下：

* 如果场上的村民数小于 19，那么每回合村民重生的概率将提升为 1 / (村民数 + 1)，但上限为 0.5。
...
游戏进行在一张 60×90 的二维的矩形地图上。地图上有若干障碍物，玩家的行动会被障碍物阻挡。
...
游戏开始时，所有参与游戏的玩家和若干村民在地图上的**随机位置**出生。
...
游戏主循环将自动触发每一回合中的计算并调用玩家编写的 AI。结算过程如下：

1. 预计算阶段
  * 存活的村民报告本回合的行动；
  * 所有玩家金钱自然增长；
2. AI 调用阶段
  * 玩家的 AI 被调用，逻辑获得玩家 AI 报告的回合动作；
  * AI 的时间限制为 150 毫秒，超时则会被强制结束；
3. 效果结算阶段
  * 守卫：过期的守卫消失；
  * 炸弹：炸弹向前飞行；到达爆炸时间的炸弹爆炸，从地图上消失，所有波及到的单位受到伤害；
...
6. 道具使用阶段
  ...
  * 玩家在指定的地点放置守卫；
7. 移动计算阶段
  * 玩家根据玩家下达的指令，或者沿着已经计算好的路径移动；
8. 重生阶段
  * 死亡惩罚结束的玩家随机取代地图上的一个村民（该村民直接消失，并且玩家出生在这个村民原先所在的位置）；如果地图上不幸没有村民，那么就在随机位置出生；
  * 村民以一定的的概率在随机位置出生；如果成功出生一位村民，那么重复上述过程，否则停止重生村民。
	* 如果村民的人数大于或等于 19，这一概率就恒定为 0.05；否则，这一概率提升为 1 / (村民数 + 1)，至多不超过 0.5。

### 胜利判定

在有至少一位玩家的分数达到 15 分或以上，或者回合数达到 1100 时，将会产生胜利者。
...
* **速度**：单位的速度向量；由于游戏机制的性质，该向量的模只能为 0 或 2.0。
...
在游戏开始时，每名玩家都将获得 15 初始金钱。

另外，玩家每 6 回合还将获得 1 金钱。金钱的自然增长与玩家是否死亡无关。

## 移动

游戏中单位可以移动。移动在回合最后、单位重生之前结算。

所有的移动都是分段的匀速直线运动，移动速度只能是每回合 2.0 单位长度。虽然这么说，因为是回合制，所以实质上都是“瞬移”。
...
村民的移动规律如下：任意选取地图上一个合法位置，沿最短路径前往彼处，原地停止 10 ~ 20 回合，然后重复。不过，在村民重生后的第一次移动之前，有 1/2 的概率先原地静止不动 1 ~ 5 回合，再开始第一次移动。

## 昼夜和视野系统

如您所知，游戏中存在昼夜系统。良好利用昼夜的特性，在白天隐藏自己、收集信息，在黑夜亮出獠牙，是取胜的关键。

* 白昼长度为 80 回合，黑夜长度为 30 回合；
* 游戏开始时为白昼的第一回合，白昼结束后立即转入黑夜，黑夜过后则立即转入白昼，如此往复。

玩家自身的视野为圆形，不受地形阻挡，但受昼夜变化影响，具体如下：

* 在白昼，玩家的视野半径为 110（拥有全地图视野）；
* 在黑夜，玩家的视野半径为 10；

另外，守卫也可以为放置它的玩家提供视野。守卫提供的视野范围与昼夜无关，始终为 15。如果玩家死亡，在死亡惩罚期间内，玩家丧失自身的视野，但玩家放置的守卫则照常提供视野。
...
* 每 8 回合，受到 5 点伤害。**所谓“每 8 回合”，是指玩家在编号为 8 的倍数，同时又是白昼的回合受到伤害（首回合的编号为 1）。**

## 普通攻击

玩家可以对身边的单位进行普通攻击。普通攻击造成 50 点伤害，并且回复自身 12 点 HP。

普通攻击的射程为 4.0，冷却时间为 12 回合。
...
* 炸弹从投出到爆炸有一个延迟，该延迟的长度固定为 3 回合；这期间，炸弹作匀速直线运动由投出点飞向起爆点；
	* 炸弹无论飞行距离多远，飞行时间均为 3 回合，炸弹的速度大小则根据需要飞行的距离发生改变；
* 炸弹的爆炸伤害为 90 点，爆炸半径为 10，受波及的单位无论与爆炸中心的距离多少，均受到等同的伤害；
* 炸弹不能对自己造成伤害。
...
* 使用守卫不受地形影响，守卫可以正常地放置在离玩家距离不超过 4 的任何一个位置，提供的视野范围不受地形阻拦；
* 守卫放置后立刻生效并且不能再移动，监视以其自身为圆心、半径 15 的一个圆 50 回合，之后消失。

炸弹和守卫的价格分别为 20 和 5 金钱。可以随时购买道具，但每种道具在使用后都将会进入冷却。炸弹的冷却时间为 40 回合，守卫的冷却时间则为 150 回合。
...
* 40 回合内处于死亡状态，一切指令无效，并且丧失玩家自身的视野（守卫不受影响）；
* 所持金钱乘以 0.3 向下取整。
...
单位若死亡，则仅当前回合对该单位造成了伤害的玩家算作击杀者。即，任何形式的助攻均不被考虑。

## 总结

通过以上的描述，您应该已经对逻辑的运作方式有了充分的了解。

如果您希望继续了解如何编写 AI，请移步 AI 编写指导教程。
